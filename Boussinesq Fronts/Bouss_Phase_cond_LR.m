function [F,J] = Bouss_Phase_cond_LR(u,p,mesh_params,jac)
...
if jac ~= 1
    ...
    [uuR_1,DuuR_1,uuR_2,DuuR_2,ur0_left] = get_rolls(p,kxp,phi,mesh_params,ur0_right);
else
...
% Right-hand side
L1 = D1*mesh_params.D2x;
L2 = D2*mesh_params.D2x;
...
nPer = 2; 
iBm = find( mesh_params.x <= -mesh_params.Lx/2 + nPer*2*pi/abs(kxm) ); 
% find indices close to x = Lx - 2*pi/kx : Lx and y = 0..Ly
x = linspace(-mesh_params.Lx/2,mesh_params.Lx/2,mesh_params.nx);  hx= x(2) - x(1);
iim= find(x <= -mesh_params.Lx/2 + nPer*2*pi/abs(kxm)); nnxm = length(iim);
...
% phase conditon \int w* du_r/dkx dx. 
...
if nargout > 1
    ...
    epsiF = 1e-6;
    dF1 = Bouss_Phase_cond_LR([u(1:2*n); u(2*n+1) + epsiF; u(2*n+2)],p,mesh_params,1);
    dF2 = Bouss_Phase_cond_LR([u(1:2*n); u(2*n+1) - epsiF; u(2*n+2)],p,mesh_params,1);
    J(:,2*n+1) = 0.5*(dF1 - dF2)/epsiF;

    dF1 = Bouss_Phase_cond_LR([u(1:2*n); u(2*n+1); u(2*n+2) + epsiF],p,mesh_params,1);
    dF2 = Bouss_Phase_cond_LR([u(1:2*n); u(2*n+1); u(2*n+2) - epsiF],p,mesh_params,1);
    J(:,2*n+2) = 0.5*(dF1 - dF2)/epsiF;
