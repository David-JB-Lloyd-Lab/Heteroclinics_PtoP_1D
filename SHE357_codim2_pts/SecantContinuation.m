...
fsolveOptionsConverge = optimset(fsolveOptions,'Display','off','Jacobian','off','MaxIter',1e3);
...
if ~isempty(ComputeEigenvalues)
    ...
    d = diag(D);
    numUnstableEigenvalues = numel( find( real(d) > 1.001 ) ); %modified for Floquet multipliers
end
...
while step < maxSteps && ~continuationFailed && ~reachedBound
    ...
    if exitflag > 0
        ...
        v0 = v1;
        v1 = v;
        % Compute parameter step size
        % dp = abs(v0(iP) - v1(iP));
        %
        % % Optionally adjust step size based on dp
        % if abs(dp) < 1e-3
        %     s = 5e-2/dp  % enlarge step
        % end
        ...
        if ~isempty(ComputeEigenvalues)
    	 ...
    	 d = diag(D);
    	 numUnstableEigenvalues = numel( find( real(d) > 1.005 ) );
        end
